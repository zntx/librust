...
struct Result
{
    ...
    unwrap_or(const U &defaultValue)
    {
        if (is_ok())
        {
            ok_ = false;
...
{
    static_assert(concept11 ::EqualityComparable<T>::value, "T must be EqualityComparable for Result to be comparable");
    static_assert(concept11 ::EqualityComparable<E>::value, "E must be EqualityComparable for Result to be comparable");
